...
- A match in a tournament room always consists of exactly 3 games. A match in a casual room consists of a single game [A7].
...
Casual (ad-hoc) rooms consist of a single game [A7]. The three-game match structure applies exclusively to tournament rooms, where complete placement data across multiple games is essential for fair top-3 advancement and tie-breaking. If the host of a casual room wants to play again, they create a new room. This simplifies the casual gameplay path while preserving the competitive rigor required by tournaments.
...
| A7 | **Design decision (qualifies FR-R19): the "best-of-three match" applies only to tournament rooms. A casual (ad-hoc) room is modelled as a single-game match.** This is not a passive assumption but an explicit narrowing of FR-R19's scope: where FR-R19 describes a match as a best-of-three series, we read it as the tournament rule (consistent with the requirement that Elo is updated *per game* and that best-of-three is described inside the tournament progression rules section). To replay, the host of a casual room creates a new room. | Two facts in the team's own requirements document push toward this scoping rather than a literal reading of FR-R19: (a) Elo is defined per *game*, not per match, which only makes sense if casual rooms are single-game; (b) the best-of-three description sits inside the tournament progression rules. Treating A7 as a *design decision that scopes FR-R19* removes the apparent internal inconsistency between FR-R19 and the Elo / tournament-progression requirements. |
